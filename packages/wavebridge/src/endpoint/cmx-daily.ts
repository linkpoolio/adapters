...
export const supportedEndpoints = ['cmx-daily']

export interface CmxIndex {
  index: number
}
...
export const execute: ExecuteWithConfig<Config> = async (request, context, config) => {
  ...
  const options = {
    ...config.api,
    params,
    url: `/api/v3/index/${config.apiCorpCode}/market/CMX10`,
  }
  const response = await Requester.request<ResponseSchema>(options)

  const cmxData = response.data.data
  if (!Array.isArray(cmxData) || cmxData.length !== 1) {
    throw new AdapterError({
      jobRunID,
      statusCode: 200,
