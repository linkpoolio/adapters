...
export async function flattenAllSchemas(): Promise<FlattenedSchema[]> {
  ...
  const pkgs = await Promise.all(
    workspacePackages.map(async (p) => {
      ...
      const collisionIgnoreMap = getCollisionIgnoreMapFrom(`${location}/env.bootstrap.json`)
...
function createChainlinkLabsResolver() {
  ...
  const bootstrapSchemaJson = require('../../env.bootstrap.json')
  schemas.push(bootstrapSchemaJson)
