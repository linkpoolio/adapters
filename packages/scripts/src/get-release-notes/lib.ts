...
export function getReleaseNotes(): string {
  ...
  changesetFileDirs.map((fileDir) => {
    ...
    const changedAdapters = fileContents.filter(
      (line) => line.indexOf('@chainlink') !== -1 || line.indexOf('@linkpool') !== -1,
    )

    changedAdapters.map((line) => {
      const adapterNameDirty = line.match(/'@chainlink\/.+'/) || line.match(/'@linkpool\/.+'/)
      ...
      const adapterNameWithoutPrefix = adapterName
        .replace('@chainlink/', '')
        .replace('@linkpool/', '')
